...
class Reptile:
    ...
    def train(self, X, y, task_lengths, num_epochs):
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            grad_list = list(gradient.values())

            idx=0
            for p in self.model.params.parameters():
                p.grad = grad_list[idx]
                idx += 1
    ...
    def _outer_step(self, model, X, y, task_lengths, epoch, num_epochs):
        ...
        for t in range(self.num_tasks):
            ...
            if t==0:
                ...
                sum_phi_minus_W = {candidate: (sum_phi_minus_W[candidate] + (phi[candidate] - weights[candidate])/0.005) for candidate in weights} # 0.005 = inner_lr * inner_epochs
